...
const MOBILE = {
  // Top pegboard
  glassPlate:   { col: 0, row: 0, cols: 6, rows: 2 },
  colorCharm:   { col: 0, row: 2 },
  resetCharm:   { col: 4, row: 2 },
  // Bottom pegboard
  ledScreen:    { col: 0, row: 0, cols: 6, rows: 4 },
  asanaCharm:   { col: 0, row: 5 },
  expediaCharm: { col: 2, row: 5 },
  linkedinCharm:{ col: 4, row: 5 },
  skillButtons: [
    { col: 0, row: 7 }, // React
    { col: 1, row: 7 }, // Python
    { col: 2, row: 7 }, // HTML
    { col: 3, row: 7 }, // CSS
    { col: 4, row: 7 }, // TypeScript
    { col: 5, row: 7 }, // AI
  ],
};

// ── Desktop skill button positions ────────────────────────────────────────────
const DESKTOP_SKILLS = [
  { col: 10, row: 3 }, // React
  { col: 11, row: 3 }, // Python
  { col: 12, row: 3 }, // HTML
  { col: 13, row: 3 }, // CSS
  { col: 14, row: 3 }, // TypeScript
  { col: 15, row: 3 }, // AI
];
...
const SKILL_BUTTONS = [
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" width="15" height="15" fill="none">
        <path d="M12 2L13.5 9.5L21 11L13.5 12.5L12 20L10.5 12.5L3 11L10.5 9.5L12 2Z" fill="#A78BFA"/>
        <path d="M19.5 3L20.2 5.8L23 6.5L20.2 7.2L19.5 10L18.8 7.2L16 6.5L18.8 5.8L19.5 3Z" fill="#C4B5FD"/>
      </svg>
    ),
  },
];
...
function BoardContent() {
  ...
  // On mobile: use fixed compact positions; on desktop: use localStorage positions
  const topGlassPlate   = isMobile ? MOBILE.glassPlate   : { col: 0, row: 0, cols: 8, rows: 2 };
  const topColorCharm   = isMobile ? MOBILE.colorCharm   : colorCharmPos;
  const topResetCharm   = isMobile ? MOBILE.resetCharm   : resetCharmPos;
  const btmLEDScreen    = isMobile ? MOBILE.ledScreen    : { col: 0, row: 0, cols: 9, rows: 6 };
  const btmAsana        = isMobile ? MOBILE.asanaCharm   : asanaCharmPos;
  const btmExpedia      = isMobile ? MOBILE.expediaCharm : expediaCharmPos;
  const btmLinkedin     = isMobile ? MOBILE.linkedinCharm: linkedinCharmPos;
  const skillPositions  = isMobile ? MOBILE.skillButtons : DESKTOP_SKILLS;
    ...
    >
      <style>{`
        @media (max-width: 768px) {
          .board-outer { padding: 1rem !important; gap: 1rem !important; }
          .board-bottom-row { flex-direction: column !important; }
          .board-pegboard-wrap { width: 100% !important; flex-shrink: 0; }
          .board-monitor-wrap { flex: none !important; width: 100% !important; height: 320px; }
        }
      `}</style>

      {/* ── Top pegboard ── */}
      <Pegboard
        className="board-pegboard-wrap"
        width="100%"
        minHeight={isMobile ? "210px" : "120px"}
      >
        <PegboardGrid>
          <GlassPlate
            text="Hello, World!"
            col={topGlassPlate.col}
            row={topGlassPlate.row}
            cols={topGlassPlate.cols}
            rows={topGlassPlate.rows}
          />
          <ColorCharm
            col={topColorCharm.col}
            row={topColorCharm.row}
            cols={2} rows={2}
            onMove={isMobile ? undefined : (col, row) => setColorCharmPos({ col, row })}
            hint="Click disc to cycle wall color"
          />
          <ResetCharm
            col={topResetCharm.col}
            row={topResetCharm.row}
            cols={2} rows={2}
            onMove={isMobile ? undefined : (col, row) => setResetCharmPos({ col, row })}
            onReset={handleReset}
        ...
        >
          <PegboardGrid>
            <LEDScreen
              ...
              col={btmLEDScreen.col}
              row={btmLEDScreen.row}
              cols={btmLEDScreen.cols}
              rows={btmLEDScreen.rows}
            />
            <AsanaCharm
              col={btmAsana.col}
              row={btmAsana.row}
              cols={2} rows={2}
              onMove={isMobile ? undefined : (col, row) => setAsanaCharmPos({ col, row })}
            ...
            <ExpediaCharm
              ...
              row={btmExpedia.row}
              cols={2} rows={2}
              onMove={isMobile ? undefined : (col, row) => setExpediaCharmPos({ col, row })}
              hint="Previously at Expedia"
            />
            {SKILL_BUTTONS.map((btn, i) => (
              <PegboardButton
                key={i}
                col={skillPositions[i].col}
                row={skillPositions[i].row}
                hint={btn.hint}
                icon={btn.icon}
              />
            ))}
            <SocialCharm
              ...
              row={btmLinkedin.row}
              cols={2} rows={2}
              onMove={isMobile ? undefined : (col, row) => setLinkedinCharmPos({ col, row })}
