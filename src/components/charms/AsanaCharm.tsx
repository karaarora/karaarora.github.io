...
export default function AsanaCharm({
  col: initialCol = 0,
  row: initialRow = 0,
  onMove,
  cols = 2,
  ...
  const CLICK_TEXT =
    "I am a Software Engineer at Asana. We build tools that help teams organize, track, and manage their work.";
  const id = useId();
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const onMouseUp = () => {
      if (wasDragging) {
        ...
        // Plain click — trigger PCB message
  ...
  return (
    ...
    >
      ...
      >
        {/* Asana logo — three dots in triangular arrangement */}
        <svg
          viewBox="0 0 50 42"
          fill="none"
        ...
        >
          {/* Top circle */}
          <circle cx="25" cy="8"  r="8"  fill="#F06A6A" />
          {/* Bottom-left circle */}
          <circle cx="8"  cy="34" r="8"  fill="#F06A6A" />
          {/* Bottom-right circle */}
          <circle cx="42" cy="34" r="8"  fill="#F06A6A" />
        </svg>
      </div>
    </div>
  );
